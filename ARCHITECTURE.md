...
ベルト経路の生成、設置可能判定、接続された燃え滓の探索を扱います。将来、通常ベルト、分配器、交差ベルトをポートグラフとして拡張します。

### `src/core/simulation.js`

炎の生成・射出・搬送・滞留・着火・延焼・納品を扱います。CanvasやDOMには触れず、納品や着火をコールバックイベントとして外部へ通知します。

### `src/core/water-simulation.js`

電気の生成・搬送、ポンプの入力容量と保持、水への変換・排出、貯水タンクへの納品を扱います。ポンプの詰まりはRendererではなく、このCore状態から自然に発生します。
