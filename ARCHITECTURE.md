...
資源共通の滞留時間、警告・故障開始・💀化、一括撤去回数を定義します。故障は資源が実際に滞留したベルトだけに発生し、接続されているだけの空ベルトへは伝播しません。設備内部の正規ストックは故障対象外ですが、設備へ誤投入された素材は入力ブロック内へ一度取り込まれ、ベルト滞留と同じ警告時間を経てから設備故障を開始します。`failureType`をRendererへ渡し、原因別の演出と共通の最終状態を両立します。
